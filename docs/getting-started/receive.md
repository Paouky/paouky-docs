...
### Slatepack
`Slatepacks` are encoded text messages used to transfer the data required to form a transaction, used as an alternative to a hands-off synchronous method such as Tor.
...
Next, your own wallet will output a beautiful `Slatepack` as well:
