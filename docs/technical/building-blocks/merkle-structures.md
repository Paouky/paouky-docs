...
There are four Merkle trees each block commits to:
...
* Support for serialization and efficient merging of pruned trees from partial archival nodes.

### Inputs and Outputs

Each object is one of two things: an input (unambiguous reference to an old
transaction output), or an output ([commitment, rangeproof] pair). It is
...
* Efficient inclusion proofs, for proof-of-publication.
