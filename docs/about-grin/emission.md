...
Even then, the stability of a chain sustaining itself through high transaction fees is questionable[^2], potentially being prone to a new set of mining attacks, all of which could be avoided given similar mining revenues earned only through block subsidy.
...
* 1 grin/s is very intuitive to understand[^5]. Such clear emission rule is more likely to be enforced by community consensus, unlike a blurred, complex model.
